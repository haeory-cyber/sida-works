...
# ==========================================
# 1. [지족 사입 전담반] (무조건 지족 탭으로 감)
# ==========================================
JIJOK_SUPPLIERS = [
    "지족점(벌크)", "지족(Y)", "지족점_공동구매", "지족점과일", 
    "지족점야채", "지족매장", "지족점정육"
]

# ==========================================
# 2. [일반 발주 업체] (일반 업체 탭으로 감)
# ==========================================
VALID_SUPPLIERS = [
...
if menu == "📦 품앗이 오더 (자동 발주)":
    ...
    if df_s is not None:
        ...
        if s_item and s_qty and s_amt:
            if s_farmer:
                # [분류 로직 개선] 지족 전담반 우선 체크
                valid_set = {v.replace(' ', '') for v in VALID_SUPPLIERS}
                jijok_set = {j.replace(' ', '') for j in JIJOK_SUPPLIERS}
                
                df_s['clean_farmer'] = df_s[s_farmer].astype(str).str.replace(' ', '')
                
                def classify(name):
                    clean = name.replace(' ', '')
                    if clean in jijok_set: return "지족(사입)"  # 1순위: 전담반 리스트
                    elif "지족" in clean: return "지족(사입)"    # 2순위: 이름에 '지족' 포함
                    elif clean in valid_set: return "일반업체" # 3순위: 일반 업체 리스트
